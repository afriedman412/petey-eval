...
def chart_model_comparison(med, par_s, out_dir):
    ...
    fig.update_layout(**LAYOUT, showlegend=False, height=530, width=1000,
                      margin=dict(t=70, b=100, l=60, r=30),
                      title=dict(text='Model Performance', y=0.98))
...
def chart_runtime(timing_df, out_dir):
    """Runtime by parser, averaged across models."""
    t_agg = (timing_df.groupby(['dataset', 'parser'])['sec_per_file']
             .mean().reset_index())

    fig = make_subplots(rows=1, cols=2, subplot_titles=[
        'Medical (1pg docs)', 'PAR Simple (~3pg docs)',
    ])
    for i, ds in enumerate(['medical', 'par_simple'], 1):
        sub = t_agg[t_agg['dataset'] == ds].sort_values('sec_per_file')
        colors = [PC.get(p, '#999') for p in sub['parser']]
        fig.add_trace(go.Bar(
            x=[dflat(p) for p in sub['parser']],
            y=sub['sec_per_file'],
            marker_color=colors,
            text=[f'{v:.1f}s' for v in sub['sec_per_file']],
            textposition='outside',
        ), row=1, col=i)
    fig.update_layout(**LAYOUT, showlegend=False, height=400, width=800,
                      margin=dict(t=70, b=80, l=60, r=30),
                      title=dict(text='Runtime per Document', y=0.98))
    fig.update_yaxes(title='Seconds per doc')
    fig.write_image(out_dir / '06_runtime.png', scale=2)
    print('  06_runtime.png')
...
def main():
    ...
    ap.add_argument('--timing', default='results/timing.csv', help='Timing CSV')
    args = ap.parse_args()
    ...
    print(f'Timing: {len(timing)} records')
